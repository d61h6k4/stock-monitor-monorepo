...
def average_true_range(df: DataFrame, p: int = 14) -> Series:
...
def last_atr(df: DataFrame, p: int = 14) -> float:
...
def moving_average_distance(df: DataFrame, fast_ma: int, slow_ma: int) -> float:
    ...
    assert "Close" in df.columns
    return (
        moving_average(df, fast_ma).last(offset="1D").max()
        / moving_average(df, slow_ma).last(offset="1D").max()
    )
...
def cot_net_position(df: DataFrame) -> DataFrame:
    ...
    cdf["commercials"] = (
        cdf["Comm_Positions_Long_All"] - cdf["Comm_Positions_Short_All"]
    )
    cdf["funds"] = (
        cdf["NonComm_Positions_Long_All"] - cdf["NonComm_Positions_Short_All"]
    )
    cdf["small traders"] = (
        cdf["NonRept_Positions_Long_All"] - cdf["NonRept_Positions_Short_All"]
    )
...
def macd(df: DataFrame) -> Series:
    ...
    assert "Close" in df.columns
    return (
        df["Close"].ewm(alpha=2.0 / 13.0).mean()
        - df["Close"].ewm(alpha=2.0 / 27.0).mean()
    )
...
def rsi(df: DataFrame) -> Series:
    ...
    average_gain = gain.ewm(alpha=2.0 / 15.0).mean()
    loss = df["Close"].pct_change().clip(upper=0.0) * -1.0
    average_loss = loss.ewm(alpha=2.0 / 15.0).mean()
    return 100 - (100 / (1.0 + average_gain / average_loss)).fillna(100.0)
...
def adx(df: DataFrame) -> DataFrame:
    ...
    pdi = 100 * (pdm.ewm(alpha=2.0 / 15.0).mean() / atr).fillna(0.0)
    ndi = abs(100 * (ndm.ewm(alpha=2.0 / 15.0).mean() / atr)).fillna(0.0)
    dx = ((abs(pdi - ndi) / abs(pdi + ndi)) * 100).fillna(0.0)
    adx_smooth = dx.ewm(alpha=2.0 / 15.0).mean()

    return (
        pd.concat([pdi, ndi, adx_smooth], axis=1)
