...
def idea_stock_graph(stock, box="1 3 11 5"):
    ...
    graph_data = dataframe_to_data(
        color_stock_dataframe(preprocess_dataframe(stock.history))[
            ...
            ui.visualization(
                ...
                plot=ui.plot(
                    [
                        ...
                        ui.mark(
                            y=stock.expectation.price,
                            label=f"Expected price {stock.expectation.price}",
                        ),
                    ]
                ),
            ),
...
async def render(q: Q):
    for ix, stock in enumerate(ideas(period="6mo", interval="1d")):
        graph = ui.form_card(
            box=f"1 {2 + 6 * ix} 11 6", items=[ui.text_s(stock.description)]
        )
