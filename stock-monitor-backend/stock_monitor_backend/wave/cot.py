...
from stock_monitor_data import COT, Stock

from stock_monitor_backend.math import cot_index, cot_move_index, cot_net_position
from stock_monitor_backend.wave.common import (
    dataframe_to_data,
...
async def render(q: Q):
    ...
    q.page["form"] = ui.form_card(
        ...
        items=[
            ui.date_picker(
                ...
                value=(q.args.cot_form_since or str(datetime.today().isoformat())),
            ),
            ui.dropdown(
                name="cot_form_names",
                label="Market and exchange names",
                choices=[
                    ui.choice(name=n.replace(" ", "_"), label=f"{n} (Code {c})")
                    for n, c in set(
                        tuple(x)
                        for x in default_cot.history[
                            ["Market_and_Exchange_Names", "CFTC_Commodity_Code"]
                        ].values
                    )
                ],
                values=(q.args.cot_form_names or []),
                required=False,
            ),
            ui.dropdown(
                name="cot_form_codes",
                label="Market and exchange names by code",
                choices=[
                ...
                values=(q.args.cot_form_codes or []),
                required=False,
            ),
            ui.slider(
                name="cot_form_period",
                label="Rolling window size (weeks)",
                min=1,
                max=3 * 365 // 7,
                step=1,
                value=(q.args.cot_form_period or 26),
            ),
            ui.textbox(
                name="cot_form_ticker",
                label="Stock ticker",
    ...
    if q.args.cot_form_ready:
        ...
        df = cot.commercials_by_names_or_codes(
            cot.history,
        ...
        cdf = (
            cot_index(df, q.args.cot_form_period)
            .to_frame(name="cot_index")
            .join(
                cot_move_index(df, q.args.cot_form_period).to_frame(
                    name="cot_move_index"
                )
            )
            .dropna()
        ...
        if q.args.cot_form_ticker:
            stock_df = Stock(
                ticker_name=q.args.cot_form_ticker, period="5y", interval="1wk"
            )
            cdf = (
                cdf.join(stock_df.history.to_period("w"))
                .to_timestamp()
                .reset_index()
                .fillna(0.0)
            )

            stock_df.history = cdf.rename(
                columns={"Report_Date_as_MM_DD_YYYY": "Date"}
            ).set_index("Date")
        ...
        net_data = dataframe_to_data(
            preprocess_dataframe(
                net_df.to_timestamp()
                .reset_index()
                .rename(columns={"Report_Date_as_MM_DD_YYYY": "Date"})[
                    ["Date", "net", "cot_type"]
                ]
                .set_index("Date")
            )
        ...
        cot_data = dataframe_to_data(
            preprocess_dataframe(
                cdf.rename(columns={"Report_Date_as_MM_DD_YYYY": "Date"})[
                    ["Date", "cot_index", "cot_move_index"]
                ].set_index("Date")
            )
        )
        q.page["cot_net"] = ui.form_card(
            box=f"4 {cot_chart_start} -1 7",
            items=[
                    ...
                    plot=ui.plot(
                        [
                            ui.mark(
                                ...
                                color_domain=["commercials", "funds", "small traders"],
                            )
                        ]
                    ),
                ),
